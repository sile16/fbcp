...
var hash_tests = []struct {
	...
	{"Min size minus 1 byte", min_thread_size_bytes - 1, 16, "1d342169a679aa0d"},
	{"Min size", min_thread_size_bytes, 16, "0194b09b08323189"},

	{"Min size + 1 byte", min_thread_size_bytes + 1, 16, "b1a2b18ef19c9425"},
	...
	{"Min size minus 1 byte", min_thread_size_bytes - 1, 1, "1d342169a679aa0d"},
	{"Min size", min_thread_size_bytes, 1, "0194b09b08323189"},

	{"Min size + 1 byte", min_thread_size_bytes + 1, 1, "5dca84a61c7cefd0"},
...
func create_tmp_file(t *testing.T, file_size int, buf []byte) {

	file_path := filepath.Join("tempdir", strconv.FormatInt(int64(file_size), 10))
	...
	for written := 0; written < file_size; {
...
func TestSpreadHash(tb *testing.T) {
	...
	for _, tc := range hash_tests {
		...
		tb.Run(name, func(t *testing.T) {
			...
			nfshash, _ := NewSpreadHash(src_ff, &Fbcp_config{
				threads:  tc.threads,
				nodes:    1,
				nodeID:   0,
				progress: false})
